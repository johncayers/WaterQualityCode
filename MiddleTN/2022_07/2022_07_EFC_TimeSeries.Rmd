...
DataFacet <- transmute(Data, DateTime, days, Light_lux, Temp_C, pH, Depth_m, SpCond_uS_cm, ODO_PerLocSat, fDOM_RFU, Chlorophyll_RFU, TAL_PC_RFU)
  ...
  # Shade = if_else(Light_lux == 0, "gray60", "gray90")
  # )
DataFacetLong <- pivot_longer(DataFacet, names_to = "variable", values_to = "measurement", Light_lux:TAL_PC_RFU)

p <- ggplot(DataFacetLong, aes(x=days)) +
  theme_linedraw() +
  # geom_rect(aes(xmin = days, xmax = lead(days), ymin = -Inf, ymax = Inf, fill = Shade)) +
  # scale_fill_identity() +
   # geom_segment(aes(x = DateTime, xend = DateTime, y = -Inf, yend = Inf, color = Light_lux)) +
   #  scale_color_gradient(low = gray.colors(1, 0.25),
...
PercChangeTibble <- unlist(sapply(DataFacet[,3:11], function(x) perc_change_fnct(x)))
...
```{r}
Parameters <- select(DataFacet, -c(DateTime, days, LightInt, Light))
